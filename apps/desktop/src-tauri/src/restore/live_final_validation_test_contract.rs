...
use crate::restore::live_linked_update_test_contract::{
...
/// Overall status of the live final validation test contract.
///
/// Safety invariants:
...
pub enum LiveFinalValidationTestContractStatus {
    /// A required safety prerequisite is missing or unsafe.
    Blocked,
    /// All contract prerequisites satisfied. No live call was made. No state
    /// was persisted. The live final validation read integration test remains
    /// separate pending work.
    EligibleButNotExecuted,
}

/// Mode for the live final validation test contract.
///
/// Safety invariants:
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
#[serde(rename_all = "camelCase")]
pub enum LiveFinalValidationTestContractPrerequisiteStatus {
...
#[serde(rename_all = "camelCase")]
pub struct LiveFinalValidationTestContractPrerequisite {
    /// Stable prerequisite identifier (e.g. `LFVTC-PRE-01`).
    pub prerequisite_id: String,
    /// Human-readable label.
    pub label: String,
    pub status: LiveFinalValidationTestContractPrerequisiteStatus,
    pub note: String,
}
...
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(rename_all = "camelCase")]
pub struct LiveFinalValidationTestContractSafetySnapshot {
    /// Write gate — always `true` (always disabled) in the current build.
    pub write_gate_disabled: bool,
    /// Whether the live linked update test contract returned `EligibleButNotExecuted`.
    pub linked_update_contract_eligible: bool,
    /// Whether the sandbox final validation adapter returned `ReadyForSandboxCall`.
...
/// Request to the live final validation test contract.
///
/// Safety invariants:
/// - No token field.
...
/// Result of the live final validation test contract.
///
...
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(rename_all = "camelCase")]
pub struct LiveFinalValidationTestContractResult {
    pub status: LiveFinalValidationTestContractStatus,
    pub mode: LiveFinalValidationTestContractMode,
    pub message: String,
    pub prerequisites: Vec<LiveFinalValidationTestContractPrerequisite>,
    pub safety_snapshot: LiveFinalValidationTestContractSafetySnapshot,
    ...
    pub app_runtime_reads_enabled: bool,
    /// Required conditions for a future live final validation read integration test.
...
const PRE_MODE: &str = "LFVTC-PRE-01";
const PRE_EXPLICIT_FLAG: &str = "LFVTC-PRE-02";
const PRE_WRITE_GATE: &str = "LFVTC-PRE-03";
const PRE_LINKED_UPDATE_CONTRACT: &str = "LFVTC-PRE-04";
const PRE_FINAL_VALIDATION_ADAPTER: &str = "LFVTC-PRE-05";
const PRE_LINKED_ADAPTER: &str = "LFVTC-PRE-06";
const PRE_RECORD_ADAPTER: &str = "LFVTC-PRE-07";
const PRE_SCHEMA_ADAPTER: &str = "LFVTC-PRE-08";
const PRE_ADAPTER_CHAIN: &str = "LFVTC-PRE-09";
const PRE_GATE_ARMING: &str = "LFVTC-PRE-10";
const PRE_SIMULATOR: &str = "LFVTC-PRE-11";
const PRE_READINESS: &str = "LFVTC-PRE-12";
...
fn required_future_live_conditions() -> Vec<String> {
    vec![
        ...
        "schema, record, and linked update test harnesses must have prepared sandbox-only state before final validation reads".to_string(),
        ...
        "only validation read operations allowed — no schema write, first-pass record create, linked update, or attachment binary operations".to_string(),
...
/// Evaluates whether a future live final validation read integration test could be
...
///   The live final validation read integration test remains separate pending work.
/// - Does not return any token, full path, record payload, raw HTTP body,
///   old/new record IDs, or attachment URL.
pub fn evaluate_live_final_validation_test_contract(
    request: &LiveFinalValidationTestContractRequest,
    schema_plan: &SchemaWriteRequestPlan,
    record_plan: &RecordWriteRequestPlan,
) -> LiveFinalValidationTestContractResult {
    // ── Mode check ─────────────────────────────────────────────────────────────
    if matches!(request.mode, LiveFinalValidationTestContractMode::Disabled) {
        return blocked_result(
            LiveFinalValidationTestContractMode::Disabled,
            false,
            false,
            false,
            false,
    ...
    // ── Explicit flag ──────────────────────────────────────────────────────────
    if !request.explicit_internal_live_final_validation_test_contract_requested {
        return blocked_result(
            LiveFinalValidationTestContractMode::SandboxIntegrationCandidate,
            false,
            false,
            false,
            false,
            ...
            PRE_EXPLICIT_FLAG,
            &format!(
                "{PRE_EXPLICIT_FLAG}: explicit_internal_live_final_validation_test_contract_requested \
    ...
    // ── Live linked update test contract probe (LFVTC-PRE-04) ─────────────────
    let linked_update_req = LiveLinkedUpdateTestContractRequest {
    ...
    let linked_update_result =
        evaluate_live_linked_update_test_contract(&linked_update_req, schema_plan, record_plan);
    let linked_update_eligible = matches!(
        linked_update_result.status,
        LiveLinkedUpdateTestContractStatus::EligibleButNotExecuted
    );
    if !linked_update_eligible {
        return blocked_result(
            LiveFinalValidationTestContractMode::SandboxIntegrationCandidate,
            false,
            false,
            ...
            false,
            false,
            PRE_LINKED_UPDATE_CONTRACT,
            &format!(
                "{PRE_LINKED_UPDATE_CONTRACT}: live linked update test contract did not return \
                 EligibleButNotExecuted. The linked update contract must be eligible \
                 before the final validation contract can report eligible."
            ),
            vec![],
        );
    }

    // ── Sandbox final validation adapter probe (LFVTC-PRE-05) ────────────────
    let fva_req = SandboxFinalValidationAdapterRequest {
    ...
    let fva_result = build_sandbox_final_validation_adapter(&fva_req, schema_plan, record_plan);
    let fva_ready = matches!(
        fva_result.status,
    ...
    if !fva_ready {
        return blocked_result(
            ...
            false,
            false,
            PRE_FINAL_VALIDATION_ADAPTER,
            &format!(
                "{PRE_FINAL_VALIDATION_ADAPTER}: sandbox final validation adapter did not return \
                 ReadyForSandboxCall. All final validation adapter prerequisites must be \
                 satisfied before this contract can report eligible."
            ),
            vec![],
        );
    }

    // ── Linked second-pass adapter probe (LFVTC-PRE-06) ──────────────────────
    ...
    // ── Record write adapter probe (LFVTC-PRE-07) ─────────────────────────────
    ...
    // ── Schema write adapter probe (LFVTC-PRE-08) ─────────────────────────────
    ...
    // ── Adapter chain runner probe (LFVTC-PRE-09) ─────────────────────────────
    ...
    // ── Gate arming probe (LFVTC-PRE-10) ──────────────────────────────────────
    ...
    // ── Enablement readiness probe (LFVTC-PRE-12) ─────────────────────────────
    ...
    let snapshot = LiveFinalValidationTestContractSafetySnapshot {
        write_gate_disabled: true,
        linked_update_contract_eligible: true,
    ...
    LiveFinalValidationTestContractResult {
        status: LiveFinalValidationTestContractStatus::EligibleButNotExecuted,
        mode: LiveFinalValidationTestContractMode::SandboxIntegrationCandidate,
        message: format!(
            "Live final validation test contract: eligibleButNotExecuted. All {total} prerequisites \
             ...
             contract_only=true. This is a contract/readiness layer only — the live final \
             validation read integration test remains separate pending work. Attachment binary \
             handling and live end-to-end restore execution remain pending separate work."
        ),
        prerequisites,
...
fn blocked_result(
    mode: LiveFinalValidationTestContractMode,
    linked_update_contract_eligible: bool,
    ...
    blocking_pre_id: &str,
    blocked_reason: &str,
    prerequisites: Vec<LiveFinalValidationTestContractPrerequisite>,
) -> LiveFinalValidationTestContractResult {
    let gate = evaluate_write_gate();
    let write_gate_disabled = matches!(gate.status, RestoreWriteEngineStatus::Disabled);
    let total = prerequisites.len();
    LiveFinalValidationTestContractResult {
        status: LiveFinalValidationTestContractStatus::Blocked,
        mode,
        message: format!(
            "Live final validation test contract is blocked at {blocking_pre_id}. \
             No Airtable API calls were made. No changes were made. \
             Runtime execution, writes, reads, and network calls remain disabled. \
             contract_only=true."
        ),
        prerequisites,
        safety_snapshot: LiveFinalValidationTestContractSafetySnapshot {
            write_gate_disabled,
            linked_update_contract_eligible,
...
fn prereq(
    id: &str,
    label: &str,
    status: LiveFinalValidationTestContractPrerequisiteStatus,
    note: &str,
) -> LiveFinalValidationTestContractPrerequisite {
    LiveFinalValidationTestContractPrerequisite {
...
fn build_prerequisites(
    ...
    sim_ok: bool,
    readiness_ok: bool,
) -> Vec<LiveFinalValidationTestContractPrerequisite> {
    let s = |ok: bool| {
        if ok {
            LiveFinalValidationTestContractPrerequisiteStatus::Ready
        } else {
            LiveFinalValidationTestContractPrerequisiteStatus::Blocked
        }
    };
    vec![
        prereq(
        ...
        prereq(
            PRE_LINKED_UPDATE_CONTRACT,
            "Linked update contract EligibleButNotExecuted",
            s(linked_update_ok),
            "live linked update test contract must return EligibleButNotExecuted.",
        ),
        prereq(
            PRE_FINAL_VALIDATION_ADAPTER,
            "Final validation adapter ReadyForSandboxCall",
            s(fva_ok),
            "sandbox final validation adapter must return ReadyForSandboxCall.",
        ),
        prereq(
            PRE_LINKED_ADAPTER,
...
mod tests {
    ...
    fn full_request() -> LiveFinalValidationTestContractRequest {
    ...
    fn disabled_request() -> LiveFinalValidationTestContractRequest {
        LiveFinalValidationTestContractRequest {
            mode: LiveFinalValidationTestContractMode::Disabled,
            explicit_internal_live_final_validation_test_contract_requested: false,
    ...
    fn default_disabled_request_returns_blocked() {
        ...
        let result = evaluate_live_final_validation_test_contract(&disabled_request(), &sp, &rp);
        assert_eq!(
            result.status,
            LiveFinalValidationTestContractStatus::Blocked
        );
    }
    ...
    fn missing_explicit_flag_returns_blocked() {
        ...
        req.explicit_internal_live_final_validation_test_contract_requested = false;
    ...
    // ── Linked update contract not eligible ────────────────────────────────────

    #[test]
    fn linked_update_contract_not_eligible_returns_blocked() {
        let sp = simple_schema_plan();
        let rp = simple_record_plan();
        let mut req = full_request();
        // Disabling mapping_coverage_sufficient blocks the linked update contract
        // (which blocks the record contract's chain runner probe).
        ...
        let reason = result.blocked_reason.unwrap_or_default();
        assert!(reason.contains(PRE_LINKED_UPDATE_CONTRACT));
    }

    // ── Final validation adapter not ready ─────────────────────────────────────

    #[test]
    fn final_validation_adapter_not_ready_returns_blocked() {
        let sp = simple_schema_plan();
        let rp = simple_record_plan();
        let mut req = full_request();
        // final_validation_enforcement_safe=false blocks the final validation adapter
        // without blocking the linked update contract (which doesn't check it at the
        // chain runner level).
    ...
    fn record_adapter_not_ready_returns_blocked() {
        ...
        let result = evaluate_live_final_validation_test_contract(&full_request(), &sp, &rp);
        assert_eq!(
            result.status,
            LiveFinalValidationTestContractStatus::Blocked
        );
        // A blocked record plan propagates through the linked update contract's
        // record write contract probe before reaching PRE_RECORD_ADAPTER.
        let reason = result.blocked_reason.unwrap_or_default();
        assert!(
            reason.contains(PRE_LINKED_UPDATE_CONTRACT) || reason.contains(PRE_RECORD_ADAPTER),
            "expected PRE_LINKED_UPDATE_CONTRACT or PRE_RECORD_ADAPTER in reason, got: {reason}"
        );
    }
    ...
    fn evaluate_write_gate_default_remains_disabled() {
        let sp = simple_schema_plan();
        let rp = simple_record_plan();
        let _ = evaluate_live_final_validation_test_contract(&full_request(), &sp, &rp);
        let gate = evaluate_write_gate();
    ...
    fn evaluate_write_gate_remains_disabled_after_blocked_result() {
        let sp = simple_schema_plan();
        let rp = simple_record_plan();
        let _ = evaluate_live_final_validation_test_contract(&disabled_request(), &sp, &rp);
        let gate = evaluate_write_gate();
        assert!(matches!(gate.status, RestoreWriteEngineStatus::Disabled));
    }
    ...
    fn app_runtime_execution_enabled_always_false() {
        let sp = simple_schema_plan();
        let rp = simple_record_plan();
        let r1 = evaluate_live_final_validation_test_contract(&full_request(), &sp, &rp);
        let r2 = evaluate_live_final_validation_test_contract(&disabled_request(), &sp, &rp);
    ...
    #[test]
    fn all_twelve_prerequisites_present_when_eligible() {
        let sp = simple_schema_plan();
        let rp = simple_record_plan();
        let result = evaluate_live_final_validation_test_contract(&full_request(), &sp, &rp);
        assert_eq!(result.total_prerequisite_count, 12);
        ...
        let ids: Vec<_> = result
            ...
            .collect();
        assert!(ids.contains(&"LFVTC-PRE-01"));
        assert!(ids.contains(&"LFVTC-PRE-05"));
        assert!(ids.contains(&"LFVTC-PRE-12"));
    }
    ...
    fn message_mentions_live_test_pending() {
        ...
        assert!(
            ...
            "message must say live final validation test remains pending, got: {}",
            result.message
        );
    }
