...
use crate::restore::live_schema_write_test_contract::{
    evaluate_live_schema_write_test_contract, LiveSchemaWriteTestContractMode,
    LiveSchemaWriteTestContractRequest, LiveSchemaWriteTestContractStatus,
};
...
/// Overall status of the live record write test contract.
///
/// Safety invariants:
...
pub enum LiveRecordWriteTestContractStatus {
    /// A required safety prerequisite is missing or unsafe.
    Blocked,
    /// All contract prerequisites satisfied. No live call was made. No state
    /// was persisted. The live record write integration test remains separate
    /// pending work.
    EligibleButNotExecuted,
}

/// Mode for the live record write test contract.
///
/// Safety invariants:
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
#[serde(rename_all = "camelCase")]
pub enum LiveRecordWriteTestContractPrerequisiteStatus {
...
#[serde(rename_all = "camelCase")]
pub struct LiveRecordWriteTestContractPrerequisite {
    /// Stable prerequisite identifier (e.g. `LRWTC-PRE-01`).
    pub prerequisite_id: String,
    /// Human-readable label.
    pub label: String,
    pub status: LiveRecordWriteTestContractPrerequisiteStatus,
    pub note: String,
}
...
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(rename_all = "camelCase")]
pub struct LiveRecordWriteTestContractSafetySnapshot {
    /// Write gate — always `true` (always disabled) in the current build.
    pub write_gate_disabled: bool,
    /// Whether the live schema write test contract returned `EligibleButNotExecuted`.
    pub schema_contract_eligible: bool,
...
/// Request to the live record write test contract.
///
/// Safety invariants:
/// - No token field.
...
pub struct LiveRecordWriteTestContractRequest {
    ...
    /// Per-field summaries forwarded to the adapter chain probe.
...
/// Result of the live record write test contract.
///
...
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(rename_all = "camelCase")]
pub struct LiveRecordWriteTestContractResult {
    pub status: LiveRecordWriteTestContractStatus,
    pub mode: LiveRecordWriteTestContractMode,
    pub message: String,
    pub prerequisites: Vec<LiveRecordWriteTestContractPrerequisite>,
    pub safety_snapshot: LiveRecordWriteTestContractSafetySnapshot,
    ...
    pub app_runtime_reads_enabled: bool,
    /// Required conditions for a future live record write integration test.
...
const PRE_MODE: &str = "LRWTC-PRE-01";
const PRE_EXPLICIT_FLAG: &str = "LRWTC-PRE-02";
const PRE_WRITE_GATE: &str = "LRWTC-PRE-03";
const PRE_SCHEMA_CONTRACT: &str = "LRWTC-PRE-04";
const PRE_RECORD_ADAPTER: &str = "LRWTC-PRE-05";
const PRE_SCHEMA_ADAPTER: &str = "LRWTC-PRE-06";
const PRE_ADAPTER_CHAIN: &str = "LRWTC-PRE-07";
const PRE_GATE_ARMING: &str = "LRWTC-PRE-08";
const PRE_SIMULATOR: &str = "LRWTC-PRE-09";
const PRE_READINESS: &str = "LRWTC-PRE-10";
...
fn required_future_live_conditions() -> Vec<String> {
    vec![
        ...
        "schema phase must already be test-created or safely represented before record writes".to_string(),
        ...
        "only first-pass record create operations allowed — no linked updates".to_string(),
        "linked record updates remain disabled — must not be enabled in this task".to_string(),
...
/// Evaluates whether a future live record write integration test could be
...
///   The live record write integration test remains separate pending work.
/// - Does not return any token, full path, record payload, raw HTTP body,
///   old/new record IDs, or attachment URL.
pub fn evaluate_live_record_write_test_contract(
    request: &LiveRecordWriteTestContractRequest,
    schema_plan: &SchemaWriteRequestPlan,
    record_plan: &RecordWriteRequestPlan,
) -> LiveRecordWriteTestContractResult {
    // ── Mode check ─────────────────────────────────────────────────────────────
    if matches!(request.mode, LiveRecordWriteTestContractMode::Disabled) {
        return blocked_result(
            LiveRecordWriteTestContractMode::Disabled,
            false,
            false,
            false,
            false,
    ...
    // ── Explicit flag ──────────────────────────────────────────────────────────
    if !request.explicit_internal_live_record_test_contract_requested {
        return blocked_result(
            LiveRecordWriteTestContractMode::SandboxIntegrationCandidate,
            false,
            false,
            false,
            false,
            ...
            PRE_EXPLICIT_FLAG,
            &format!(
                "{PRE_EXPLICIT_FLAG}: explicit_internal_live_record_test_contract_requested \
    ...
    // ── Live schema write contract probe (LRWTC-PRE-04) ───────────────────────
    let schema_contract_req = LiveSchemaWriteTestContractRequest {
    ...
    let schema_contract_result =
        evaluate_live_schema_write_test_contract(&schema_contract_req, schema_plan, record_plan);
    let schema_contract_eligible = matches!(
    ...
    if !schema_contract_eligible {
        return blocked_result(
            ...
            PRE_SCHEMA_CONTRACT,
            &format!(
                "{PRE_SCHEMA_CONTRACT}: live schema write test contract did not return \
                 EligibleButNotExecuted. The schema write contract must be eligible \
                 before the record write contract can report eligible."
            ),
            vec![],
        );
    }

    // ── Record write adapter probe (LRWTC-PRE-05) ─────────────────────────────
    ...
    // ── Schema write adapter probe (LRWTC-PRE-06) ─────────────────────────────
    ...
    // ── Adapter chain runner probe (LRWTC-PRE-07) ─────────────────────────────
    ...
    // ── Gate arming probe (LRWTC-PRE-08) ──────────────────────────────────────
    ...
    // ── Enablement readiness probe (LRWTC-PRE-10) ─────────────────────────────
    ...
    let prerequisites =
        build_prerequisites(true, true, true, true, true, true, true, true, true, true);
    let total = prerequisites.len();
    let snapshot = LiveRecordWriteTestContractSafetySnapshot {
        write_gate_disabled: true,
        schema_contract_eligible: true,
    ...
    LiveRecordWriteTestContractResult {
        status: LiveRecordWriteTestContractStatus::EligibleButNotExecuted,
        mode: LiveRecordWriteTestContractMode::SandboxIntegrationCandidate,
        message: format!(
            "Live record write test contract: eligibleButNotExecuted. All {total} prerequisites \
             ...
             contract_only=true. This is a contract/readiness layer only — the live record \
             write integration test remains separate pending work. Linked record updates, \
             final validation reads, attachment handling, and live end-to-end restore execution \
...
fn blocked_result(
    mode: LiveRecordWriteTestContractMode,
    schema_contract_eligible: bool,
    ...
    blocking_pre_id: &str,
    blocked_reason: &str,
    prerequisites: Vec<LiveRecordWriteTestContractPrerequisite>,
) -> LiveRecordWriteTestContractResult {
    let gate = evaluate_write_gate();
    let write_gate_disabled = matches!(gate.status, RestoreWriteEngineStatus::Disabled);
    let total = prerequisites.len();
    LiveRecordWriteTestContractResult {
        status: LiveRecordWriteTestContractStatus::Blocked,
        mode,
        message: format!(
            "Live record write test contract is blocked at {blocking_pre_id}. \
             No Airtable API calls were made. No changes were made. \
             Runtime execution, writes, reads, and network calls remain disabled. \
             contract_only=true."
        ),
        prerequisites,
        safety_snapshot: LiveRecordWriteTestContractSafetySnapshot {
            write_gate_disabled,
...
fn prereq(
    id: &str,
    label: &str,
    status: LiveRecordWriteTestContractPrerequisiteStatus,
    note: &str,
) -> LiveRecordWriteTestContractPrerequisite {
    LiveRecordWriteTestContractPrerequisite {
...
fn build_prerequisites(
    ...
    sim_ok: bool,
    readiness_ok: bool,
) -> Vec<LiveRecordWriteTestContractPrerequisite> {
    let s = |ok: bool| {
        if ok {
            LiveRecordWriteTestContractPrerequisiteStatus::Ready
        } else {
            LiveRecordWriteTestContractPrerequisiteStatus::Blocked
        }
    };
    vec![
        prereq(
        ...
        prereq(
            ...
            s(flag_ok),
            "explicit_internal_live_record_test_contract_requested must be true.",
        ),
        prereq(
            PRE_WRITE_GATE,
        ...
        prereq(
            PRE_SCHEMA_CONTRACT,
            "Schema write contract EligibleButNotExecuted",
            s(schema_contract_ok),
            "live schema write test contract must return EligibleButNotExecuted.",
        ),
        prereq(
            PRE_RECORD_ADAPTER,
...
mod tests {
    ...
    fn full_request() -> LiveRecordWriteTestContractRequest {
    ...
    fn disabled_request() -> LiveRecordWriteTestContractRequest {
        LiveRecordWriteTestContractRequest {
            mode: LiveRecordWriteTestContractMode::Disabled,
            explicit_internal_live_record_test_contract_requested: false,
    ...
    fn default_disabled_request_returns_blocked() {
        ...
        let result = evaluate_live_record_write_test_contract(&disabled_request(), &sp, &rp);
        assert_eq!(result.status, LiveRecordWriteTestContractStatus::Blocked);
    }
    ...
    fn missing_explicit_flag_returns_blocked() {
        ...
        req.explicit_internal_live_record_test_contract_requested = false;
    ...
    // ── Schema contract not eligible ───────────────────────────────────────────

    #[test]
    fn schema_contract_not_eligible_returns_blocked() {
        let sp = simple_schema_plan();
        let rp = simple_record_plan();
        let mut req = full_request();
        // Disabling mapping_coverage_sufficient blocks the schema contract's chain
        // runner probe (LSWTC-PRE-05), causing it to return Blocked instead of
        // EligibleButNotExecuted.
        req.mapping_coverage_sufficient = false;
        let result = evaluate_live_record_write_test_contract(&req, &sp, &rp);
        assert_eq!(result.status, LiveRecordWriteTestContractStatus::Blocked);
        let reason = result.blocked_reason.unwrap_or_default();
        assert!(reason.contains(PRE_SCHEMA_CONTRACT));
    }
    ...
    fn record_adapter_not_ready_returns_blocked() {
        ...
        let result = evaluate_live_record_write_test_contract(&full_request(), &sp, &rp);
        assert_eq!(result.status, LiveRecordWriteTestContractStatus::Blocked);
        // A blocked record plan is consumed by the schema contract probe's chain
        // runner (LSWTC-PRE-05) before reaching PRE_RECORD_ADAPTER directly.
        // Both paths return Blocked — the check ID may be PRE_SCHEMA_CONTRACT or
        // PRE_RECORD_ADAPTER depending on evaluation order.
        let reason = result.blocked_reason.unwrap_or_default();
        assert!(
            reason.contains(PRE_SCHEMA_CONTRACT) || reason.contains(PRE_RECORD_ADAPTER),
            "expected PRE_SCHEMA_CONTRACT or PRE_RECORD_ADAPTER in blocked reason, got: {reason}"
        );
    }
    ...
    fn schema_adapter_not_ready_returns_blocked() {
        ...
        let rp = simple_record_plan();
        // A blocked schema plan also blocks the schema contract (PRE_SCHEMA_CONTRACT
        // fires before PRE_SCHEMA_ADAPTER). Both are Blocked results.
        let result = evaluate_live_record_write_test_contract(&full_request(), &sp, &rp);
        assert_eq!(result.status, LiveRecordWriteTestContractStatus::Blocked);
    }
    ...
    fn adapter_chain_not_ready_returns_blocked() {
        ...
        let rp = simple_record_plan();
        let mut req = full_request();
        // mapping_coverage_sufficient=false blocks the linked adapter inside the
        // chain runner (SACR-CHK-07), but also blocks the schema contract's chain
        // probe at PRE_SCHEMA_CONTRACT which fires first.
        // Both paths return Blocked — the test verifies the final status.
        req.mapping_coverage_sufficient = false;
        let result = evaluate_live_record_write_test_contract(&req, &sp, &rp);
        assert_eq!(result.status, LiveRecordWriteTestContractStatus::Blocked);
    }
    ...
    fn evaluate_write_gate_default_remains_disabled() {
        let sp = simple_schema_plan();
        let rp = simple_record_plan();
        let _ = evaluate_live_record_write_test_contract(&full_request(), &sp, &rp);
        let gate = evaluate_write_gate();
    ...
    fn evaluate_write_gate_remains_disabled_after_blocked_result() {
        let sp = simple_schema_plan();
        let rp = simple_record_plan();
        let _ = evaluate_live_record_write_test_contract(&disabled_request(), &sp, &rp);
        let gate = evaluate_write_gate();
        assert!(matches!(gate.status, RestoreWriteEngineStatus::Disabled));
    }
    ...
    fn app_runtime_execution_enabled_always_false() {
        let sp = simple_schema_plan();
        let rp = simple_record_plan();
        let r1 = evaluate_live_record_write_test_contract(&full_request(), &sp, &rp);
        let r2 = evaluate_live_record_write_test_contract(&disabled_request(), &sp, &rp);
    ...
    fn required_future_live_conditions_reported_when_eligible() {
        ...
        assert!(conditions.contains("linked record updates remain disabled"));
    ...
    #[test]
    fn all_ten_prerequisites_present_when_eligible() {
        let sp = simple_schema_plan();
        let rp = simple_record_plan();
        let result = evaluate_live_record_write_test_contract(&full_request(), &sp, &rp);
        assert_eq!(result.total_prerequisite_count, 10);
        assert_eq!(result.prerequisites.len(), 10);
        let ids: Vec<_> = result
            .prerequisites
            .iter()
            .map(|p| p.prerequisite_id.as_str())
            .collect();
        assert!(ids.contains(&"LRWTC-PRE-01"));
        assert!(ids.contains(&"LRWTC-PRE-10"));
    }
    ...
    fn message_mentions_live_test_pending() {
        ...
        assert!(
            ...
            "message must say live record write test remains pending, got: {}",
            result.message
        );
    }

    // ── No linked update / attachment / final validation in future conditions ─

    #[test]
    fn linked_updates_remain_disabled_in_future_conditions() {
        let sp = simple_schema_plan();
        let rp = simple_record_plan();
        let result = evaluate_live_record_write_test_contract(&full_request(), &sp, &rp);
        let conditions = result.required_future_live_conditions.join(" ");
