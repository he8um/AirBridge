...
/// Overall status of the live schema write test contract.
///
/// Safety invariants:
...
/// - The live schema write integration test itself remains separate pending work.
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
#[serde(rename_all = "camelCase")]
pub enum LiveSchemaWriteTestContractStatus {
    /// A required safety prerequisite is missing or unsafe.
    Blocked,
    /// All contract prerequisites satisfied. No live call was made. No state
    /// was persisted. The live schema write integration test remains separate
    /// pending work.
    EligibleButNotExecuted,
}

/// Mode for the live schema write test contract.
///
/// Safety invariants:
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
#[serde(rename_all = "camelCase")]
pub enum LiveSchemaWriteTestContractPrerequisiteStatus {
...
#[serde(rename_all = "camelCase")]
pub struct LiveSchemaWriteTestContractPrerequisite {
    /// Stable prerequisite identifier (e.g. `LSWTC-PRE-01`).
    pub prerequisite_id: String,
    /// Human-readable label.
    pub label: String,
    pub status: LiveSchemaWriteTestContractPrerequisiteStatus,
    pub note: String,
}
...
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(rename_all = "camelCase")]
pub struct LiveSchemaWriteTestContractSafetySnapshot {
...
/// Request to the live schema write test contract.
///
/// Safety invariants:
/// - No token field.
...
/// Result of the live schema write test contract.
///
...
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(rename_all = "camelCase")]
pub struct LiveSchemaWriteTestContractResult {
    pub status: LiveSchemaWriteTestContractStatus,
    pub mode: LiveSchemaWriteTestContractMode,
    pub message: String,
    pub prerequisites: Vec<LiveSchemaWriteTestContractPrerequisite>,
    pub safety_snapshot: LiveSchemaWriteTestContractSafetySnapshot,
    ...
    pub app_runtime_reads_enabled: bool,
    /// Required conditions for a future live schema write integration test.
...
const PRE_MODE: &str = "LSWTC-PRE-01";
const PRE_EXPLICIT_FLAG: &str = "LSWTC-PRE-02";
const PRE_WRITE_GATE: &str = "LSWTC-PRE-03";
const PRE_SCHEMA_ADAPTER: &str = "LSWTC-PRE-04";
const PRE_ADAPTER_CHAIN: &str = "LSWTC-PRE-05";
const PRE_GATE_ARMING: &str = "LSWTC-PRE-06";
const PRE_SIMULATOR: &str = "LSWTC-PRE-07";
const PRE_READINESS: &str = "LSWTC-PRE-08";

// ── Required future-live conditions ──────────────────────────────────────────

fn required_future_live_conditions() -> Vec<String> {
    vec![
        "sandbox-only base required — no production base may be used".to_string(),
        "target base must be verified empty before any schema write is attempted".to_string(),
        "explicit test-only credentials required — no token is accepted by this contract"
            .to_string(),
        "no UI execution path allowed — live call must be a separate Rust-internal task"
            .to_string(),
        "only schema operations allowed in the first live phase (createTable, createField)"
            .to_string(),
        "record writes remain disabled — must not be enabled in this task".to_string(),
...
/// Evaluates whether a future live schema write integration test could be
...
///   The live schema write integration test remains separate pending work.
/// - Does not return any token, full path, record payload, raw HTTP body,
///   old/new record IDs, or attachment URL.
pub fn evaluate_live_schema_write_test_contract(
    request: &LiveSchemaWriteTestContractRequest,
    schema_plan: &SchemaWriteRequestPlan,
    record_plan: &RecordWriteRequestPlan,
) -> LiveSchemaWriteTestContractResult {
    // ── Mode check ─────────────────────────────────────────────────────────────
    if matches!(request.mode, LiveSchemaWriteTestContractMode::Disabled) {
        return blocked_result(
            LiveSchemaWriteTestContractMode::Disabled,
            false,
            false,
            false,
            false,
    ...
    // ── Explicit flag ──────────────────────────────────────────────────────────
    if !request.explicit_internal_live_schema_test_contract_requested {
        return blocked_result(
            LiveSchemaWriteTestContractMode::SandboxIntegrationCandidate,
            false,
            false,
            false,
            false,
            ...
            PRE_EXPLICIT_FLAG,
            &format!(
                "{PRE_EXPLICIT_FLAG}: explicit_internal_live_schema_test_contract_requested \
    ...
    // ── Schema write adapter probe (LSWTC-PRE-04) ─────────────────────────────
    ...
    // ── Adapter chain runner probe (LSWTC-PRE-05) ─────────────────────────────
    ...
    // ── Gate arming probe (LSWTC-PRE-06) ──────────────────────────────────────
    ...
    // ── Enablement readiness probe (LSWTC-PRE-08) ─────────────────────────────
    ...
    let prerequisites = build_prerequisites(true, true, true, true, true, true, true, true);
    let total = prerequisites.len();
    let snapshot = LiveSchemaWriteTestContractSafetySnapshot {
    ...
    LiveSchemaWriteTestContractResult {
        status: LiveSchemaWriteTestContractStatus::EligibleButNotExecuted,
        mode: LiveSchemaWriteTestContractMode::SandboxIntegrationCandidate,
        message: format!(
            "Live schema write test contract: eligibleButNotExecuted. All {total} prerequisites \
             ...
             contract_only=true. This is a contract/readiness layer only — the live schema \
             write integration test remains separate pending work. Record writes, linked record \
             updates, final validation reads, and live end-to-end restore execution remain \
...
fn blocked_result(
    mode: LiveSchemaWriteTestContractMode,
    ...
    blocking_pre_id: &str,
    blocked_reason: &str,
    prerequisites: Vec<LiveSchemaWriteTestContractPrerequisite>,
) -> LiveSchemaWriteTestContractResult {
    let gate = evaluate_write_gate();
    let write_gate_disabled = matches!(gate.status, RestoreWriteEngineStatus::Disabled);
    let total = prerequisites.len();
    LiveSchemaWriteTestContractResult {
        status: LiveSchemaWriteTestContractStatus::Blocked,
        mode,
        message: format!(
            "Live schema write test contract is blocked at {blocking_pre_id}. \
             No Airtable API calls were made. No changes were made. \
             Runtime execution, writes, reads, and network calls remain disabled. \
             contract_only=true."
        ),
        prerequisites,
        safety_snapshot: LiveSchemaWriteTestContractSafetySnapshot {
            write_gate_disabled,
...
fn prereq(
    id: &str,
    label: &str,
    status: LiveSchemaWriteTestContractPrerequisiteStatus,
    note: &str,
) -> LiveSchemaWriteTestContractPrerequisite {
    LiveSchemaWriteTestContractPrerequisite {
...
fn build_prerequisites(
    ...
    sim_ok: bool,
    readiness_ok: bool,
) -> Vec<LiveSchemaWriteTestContractPrerequisite> {
    let s = |ok: bool| {
        if ok {
            LiveSchemaWriteTestContractPrerequisiteStatus::Ready
        } else {
            LiveSchemaWriteTestContractPrerequisiteStatus::Blocked
        }
    };
    vec![
        prereq(
        ...
        prereq(
            ...
            s(flag_ok),
            "explicit_internal_live_schema_test_contract_requested must be true.",
        ),
        prereq(
            PRE_WRITE_GATE,
...
mod tests {
    ...
    fn full_request() -> LiveSchemaWriteTestContractRequest {
    ...
    fn disabled_request() -> LiveSchemaWriteTestContractRequest {
        LiveSchemaWriteTestContractRequest {
            mode: LiveSchemaWriteTestContractMode::Disabled,
            explicit_internal_live_schema_test_contract_requested: false,
    ...
    fn default_disabled_request_returns_blocked() {
        ...
        let result = evaluate_live_schema_write_test_contract(&disabled_request(), &sp, &rp);
        assert_eq!(result.status, LiveSchemaWriteTestContractStatus::Blocked);
    }
    ...
    fn missing_explicit_flag_returns_blocked() {
        ...
        req.explicit_internal_live_schema_test_contract_requested = false;
    ...
    fn schema_adapter_not_ready_returns_blocked() {
        ...
        let rp = simple_record_plan();
        let result = evaluate_live_schema_write_test_contract(&full_request(), &sp, &rp);
        assert_eq!(result.status, LiveSchemaWriteTestContractStatus::Blocked);
        let reason = result.blocked_reason.unwrap_or_default();
        assert!(reason.contains(PRE_SCHEMA_ADAPTER));
    }
    ...
    fn adapter_chain_not_ready_returns_blocked() {
        ...
        let rp = simple_record_plan();
        let mut req = full_request();
        // mapping_coverage_sufficient=false causes the linked adapter inside the
        // chain runner to block, so the chain runner returns Blocked.
        req.mapping_coverage_sufficient = false;
        let result = evaluate_live_schema_write_test_contract(&req, &sp, &rp);
        assert_eq!(result.status, LiveSchemaWriteTestContractStatus::Blocked);
        let reason = result.blocked_reason.unwrap_or_default();
        assert!(reason.contains(PRE_ADAPTER_CHAIN));
    }
    ...
    fn gate_arming_not_ready_returns_blocked() {
        ...
        let rp = simple_record_plan();
        let mut req = full_request();
        // rollback_limitation_safe=false causes the arming probe to block.
        // Note: arming shares prereq booleans with the chain runner; to reach
        // PRE_GATE_ARMING specifically, the chain runner must still return
        // MockRunNotExecuted. The chain runner uses mapping_coverage_sufficient
        // independently — setting rollback_limitation_safe=false will block the
        // chain runner's internal simulator first (SACR-CHK-04). This means the
        // check we observe will be PRE_ADAPTER_CHAIN, not PRE_GATE_ARMING.
        // To isolate the arming check we need a prereq that is consumed by the
        // arming probe but NOT by the chain runner's simulator. However, the
        // arming probe's prereqs are a strict subset of the simulator's prereqs,
        // so any arming failure will also fail the chain runner's simulator probe.
        // Therefore: we only assert Blocked here, not a specific prereq ID.
    ...
    fn simulator_not_ready_returns_blocked() {
        ...
        let rp = simple_record_plan();
        let mut req = full_request();
        // failure_modes_safe=false causes the simulator to block.
        // Same ordering note as the arming test above.
    ...
    fn readiness_not_ready_returns_blocked() {
        ...
        let rp = simple_record_plan();
        let mut req = full_request();
        // failure_modes_safe=false flows into the readiness harness probe AND the
        // chain runner's internal simulator. Both block; the chain runner fires
        // first at PRE_ADAPTER_CHAIN (earlier in the ordered check chain). The
        // test verifies the result is always Blocked — not which prereq blocked.
        // PRE_READINESS is an invariant-level check; its dedicated isolation is
        // impossible from a single prereq flip because the readiness probe and
        // chain runner share the same policy-boolean set.
        req.failure_modes_safe = false;
        let result = evaluate_live_schema_write_test_contract(&req, &sp, &rp);
        assert_eq!(result.status, LiveSchemaWriteTestContractStatus::Blocked);
    }
    ...
    fn evaluate_write_gate_default_remains_disabled() {
        let sp = simple_schema_plan();
        let rp = simple_record_plan();
        let _ = evaluate_live_schema_write_test_contract(&full_request(), &sp, &rp);
        let gate = evaluate_write_gate();
    ...
    fn evaluate_write_gate_remains_disabled_after_blocked_result() {
        let sp = simple_schema_plan();
        let rp = simple_record_plan();
        let _ = evaluate_live_schema_write_test_contract(&disabled_request(), &sp, &rp);
        let gate = evaluate_write_gate();
        assert!(matches!(gate.status, RestoreWriteEngineStatus::Disabled));
    }
    ...
    fn app_runtime_execution_enabled_always_false() {
        let sp = simple_schema_plan();
        let rp = simple_record_plan();
        let r1 = evaluate_live_schema_write_test_contract(&full_request(), &sp, &rp);
        let r2 = evaluate_live_schema_write_test_contract(&disabled_request(), &sp, &rp);
    ...
    fn required_future_live_conditions_reported_when_eligible() {
        ...
        assert!(conditions.contains("sandbox-only base required"));
        assert!(conditions.contains("record writes remain disabled"));
    ...
    #[test]
    fn all_eight_prerequisites_present_when_eligible() {
        let sp = simple_schema_plan();
        let rp = simple_record_plan();
        let result = evaluate_live_schema_write_test_contract(&full_request(), &sp, &rp);
        assert_eq!(result.total_prerequisite_count, 8);
        assert_eq!(result.prerequisites.len(), 8);
        let ids: Vec<_> = result
            .prerequisites
            .iter()
            .map(|p| p.prerequisite_id.as_str())
            .collect();
        assert!(ids.contains(&"LSWTC-PRE-01"));
        assert!(ids.contains(&"LSWTC-PRE-08"));
    }
    ...
    fn message_mentions_live_test_pending() {
        ...
        assert!(
            ...
            "message must say live schema write test remains pending, got: {}",
            result.message
        );
    }
}
