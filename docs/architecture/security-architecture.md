...
Security boundaries include credential storage, log sanitization, backup redaction, package validation, and restore confirmation. Tokens must never enter backup packages or logs. Restore operations require explicit user confirmation after the plan is displayed.
