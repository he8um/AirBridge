...
The system consists of the desktop app, local state database, operating system credential store, `.airbridge` package files, and Airtable APIs. All backup and restore jobs run locally. Airtable is contacted only for user-initiated connection, backup, permission check, and restore workflows.
