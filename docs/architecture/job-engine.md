...
Backup and restore are long-running jobs. A job should include ID, type, status, current step, progress, logs, warnings, errors, start time, end time, cancellation state, and result summary. Jobs should emit progress events to the frontend and store summary history in SQLite.
