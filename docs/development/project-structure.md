...
The repository uses an app-focused structure under `apps/desktop`, with frontend source in `src` and Tauri/Rust source in `src-tauri/src`. Documentation lives in `docs`, issue templates in `.github`, and examples in `examples`.
